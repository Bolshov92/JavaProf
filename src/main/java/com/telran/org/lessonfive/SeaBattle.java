...
public class SeaBattle {
    ...
    private static int verificationShot(char[][] battleField, char[][] field, int x, int y) {
        if (battleField[y][x] == 'X') {
            System.out.println("This cell has already been shot. Try again.");
            return -1; // Уже стреляли в эту ячейку
        } else if (field[y][x] == '=') {
            System.out.println("It's a hit!");
            battleField[y][x] = 'X'; // Отмечаем успешный выстрел на поле противника
            return 1; // Успешный выстрел
        } else {
            System.out.println("Missed!");
            battleField[y][x] = 'O'; // Отмечаем промах на поле противника
